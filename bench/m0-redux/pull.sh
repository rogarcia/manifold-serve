...
# Usage: pull.sh HOST PORT [RUN]     RUN (e.g. 2026-09-24) puts the manifests in results/RUN/
set -euo pipefail
...
MAN="$D/results${3:+/$3}"; mkdir -p "$MAN"
scp -i "$KEY" -P "$PORT" -q "root@$HOST:/workspace/m0-redux/env-manifest.txt" "root@$HOST:/workspace/m0-redux/pip-freeze.txt" "$MAN/"
